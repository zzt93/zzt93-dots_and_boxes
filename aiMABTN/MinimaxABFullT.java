...
public class MinimaxABFullT implements MinimaxInterface {
	int visit = 0;
	static Move bestMove;
	Node rootNode;
	static HashMap<Board, Double> trans = new HashMap<Board, Double>();
	...
	private double miniMax(Node currentNode, int depth, double alpha, double beta) 
	{
	    ...
	    if(currentNode.ai.isTurn)
	    {
	        while(moves.moreMoves())
	        {
	        	...
	        	if (trans.containsKey(currentNode.newB)) {
					alpha = trans.get(currentNode.newB) + 2*(rootNode.ai.score - rootNode.man.score);
					posess = true;
				}

				if (!posess) {
		            alpha = Math.max(alpha, miniMax(currentNode, depth - 1, alpha, beta));
		            trans.put(new Board(currentNode.state), alpha - 2*(rootNode.ai.score - rootNode.man.score));
				}
	    ...
	    else
	    {
	    	while(moves.moreMoves())
	        {
	    		...
	    		if (trans.containsKey(currentNode.newB)) {
					beta =  trans.get(currentNode.newB) + 2*(rootNode.ai.score - rootNode.man.score);
					posess = true;
				}
				if (!posess) {
		            beta = Math.min(beta, miniMax(currentNode, depth - 1, alpha, beta));
		            trans.put(new Board(currentNode.state), beta - 2*(rootNode.ai.score - rootNode.man.score));

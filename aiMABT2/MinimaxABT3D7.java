...
/**
 * use the TriMove as the key and not make use of the hashcode
 * for I iterate the hashmap myself.
 * not efficient at all, even slower
 * 
 * depth = 7
 */
...
public class MinimaxABT3D7 implements MinimaxInterface {
	static Move bestMove;
	HashMap<TriMove, Double> trans = new HashMap<TriMove, Double>();
	...
	private double miniMax(Node currentNode, int depth, double alpha, double beta, Move lastm) 
	{
	    ...
	    if(currentNode.ai.isTurn)
	    {
	        while(moves.moreMoves())
	        {
	        	...
	        	if (depth == 6) {
		            ...
		            alpha = Math.max(alpha, miniMax(currentNode, depth - 1, alpha, beta, new TwoMove(lastm, m)));
		            currentNode.retractMove(m);
				}else if (depth == 5) {        		
					for (TriMove t : trans.keySet()) {
						if (t.equal((TwoMove)lastm, m)) {							
							temp = trans.get(t);
							alpha = temp;
							posess = true;
						}
					...
					if (!posess) {
			            ...
			            trans.put(new TriMove(lastm, m), alpha);
			            currentNode.retractMove(m);
					}
				}		    	
	    		else {
	    ...
	    else
	    {
	    	while(moves.moreMoves())
	        {
	    		...
	    		if (depth == 6) {
	    			currentNode.applyMove(m, null);
		            beta = Math.min(beta, miniMax(currentNode, depth - 1, alpha, beta, new TwoMove(lastm, m)));
		            currentNode.retractMove(m);
				}else if (depth == 5) {        		
					for (TriMove t : trans.keySet()) {
						if (t.equal((TwoMove)lastm, m)) {							
					...
					if (!posess) {
			            ...
			            trans.put(new TriMove(lastm, m), beta);
			            currentNode.retractMove(m);
					}
				}else {
	            	currentNode.applyMove(m, null);
